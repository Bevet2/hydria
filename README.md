...
2. Code question:
   `J'ai une TypeError dans Node.js quand j'appelle map sur undefined. Donne une stratégie de debug.`
3. Weather:
   `Quelle est la météo à Paris aujourd'hui ?`
4. Crypto + analysis:
   `Donne le prix du BTC et fais une analyse rapide du contexte.`
5. News + summary:
   `Quelles sont les actualités IA récentes et résume-les.`
6. Image OCR:
   Upload an image containing text and ask `Que contient cette image ?`
7. PDF / DOCX summary:
   Upload a document and ask `Résume ce document`
8. Code file review:
...
10. Preference memory:
   `Réponds en français.` then save preferences in the sidebar and ask a new question.
