...
- `POST /api/hydria/control` - local exchange: send Hydria OS workspace context to Hydria Core, receive public API `proposedActions` when available, then execute allowed actions locally; falls back to the internal Core manifest endpoint if needed
- `POST /api/hydria/actions` - direct action execution endpoint for a trusted external controller; requires `HYDRIA_CONTROL_TOKEN`
- `GET /api/users`
- `POST /api/users`
